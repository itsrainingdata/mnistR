...
autoloadMNIST <- function() {
    mnistR::downloadMNIST(dest = paste0(getwd(), "/"))
    mnist_list <- mnistR::loadMNIST(dest = paste0(getwd(), "/"))
    mnist_trn <<- mnist_list$train
    mnist_tst <<- mnist_list$test
}
